Status: Partial — Remaining: rework to user+device. **This plan built the DEVICE-ONLY identity (S1–S7, ADR-0016) — now SUPERSEDED by ADR-0017 (user+device).** On-device testing showed real users have many devices and the per-device recovery handle 409s a 2nd device. Current model + frozen API: **`ARCHITECTURE.md`** (the single source). The user+device rework + the Box-first trim are the active sprint (`PLAN_box_demo_sprint.md`). S1–S7 device-only landed (backend 615 + Flutter 262 tests green) and its crypto/device-key is reused under the user; what changes is adding the user tier, user-first onboarding, and user-level membership.
<!-- Original S1–S7 status preserved below for history. -->
S1–S7 done (2026-06-18, all merged to main). S6 ping over box (Router `POST /channels/{channelId}/ping` → one-way `box.ping` fan-out to members holding a live `/v1/agent-link`; member-gated; the Flutter client registers the receive-link via a hello frame). S7 recovery: zero-knowledge seed escrow (`POST`/`GET /recovery/escrow` — server stores only opaque ciphertext + the public key, never the seed) + the Flutter recovery client (deviceId derived from the public key, Argon2id+AES-256-GCM seed wraps under password & one-time OMG code, OMG show-once screen, escrow→recover round-trip restoring the same identity). Backend 615 + Flutter 262 tests, all green.
