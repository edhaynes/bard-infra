...
### INFRA-9 — OpenTofu container stack (declarative rootless Podman on the GPU fleet)

- **Added:** 2026-06-29 · **Status:** In Progress
- **Where:** `terraform/` (OpenTofu, `kreuzwerker/docker` provider over SSH to
  the rootless Podman socket). Host GPU prep in `terraform/scripts/host-prep.sh`.

Declarative IaC for the containers that run on the GPU hosts, so services are
reproducible and reviewable rather than hand-`podman run`. **Foundation landed
2026-06-29:** provider → `ssh://ehaynes@gx10/run/user/1000/podman/podman.sock`,
parameterized host/socket (gx10 today, second host via variables/alias), and a
proven GPU passthrough path (CDI via nvidia default-runtime + `NVIDIA_VISIBLE_DEVICES`)
...
| 90 | **Fabric: contribute spare LLM cycles toward training a model ("Mega Vulcan") over bardnet (Eddie 2026-06-27)** — use the Bard fabric so fleet nodes donate spare cycles to a *training* run. Contribution rides the existing **plugin-over-Podman** model (the agent's containerized `InferenceEngine`). **Mechanism NOT settled — do not guess.** Jason's standing architecture position: bard-infra is an inference-dispatch fabric, not a trainer; if scoped, bring the algorithm from existing low-comm distributed-training OSS (DiLoCo/OpenDiLoCo, Hivemind, PETALS) and use bard-infra as the secure mesh substrate + node registry — do NOT write a training plane into this repo. Hard constraints: training is communication-bound (needs low-comm methods over a mesh); spare *inference* cycles ≠ training memory (~3–4× more); one real training-capable node today (Gladius/gx10, 128 GB unified, ARM64). The trained model lands back as an `InferenceEngine` plugin. <br>Clarify: model + size + full-FT/LoRA/continued-pretrain (decides if it fits Gladius)? does "help" mean just the compute box, or bard-infra orchestrates/exposes the run? See JOURNAL.md 2026-06-27 thread. | 2026-06-27 | Open — "Run"-horizon, not scoped; downstream of the Box MVP |
| 91 | **Management screen for the fabric (Eddie 2026-06-28)** — a real management UI beyond today's *read-only* console + plugin manager (#64/#65, v1.5.5/1.5.6). One pane to see the fleet (nodes, status, capabilities, heartbeats), approve/revoke/rename devices, manage plugins, and watch dispatch/health. Builds on the existing `clients/console/` surface + the Prometheus `/metrics` already emitted; likely **demo-driven** (pairs with "is bard-infra ready to show?"). <br>Clarify (Jason): web dashboard vs the in-client Box screen vs both? read-mostly status pane or full admin actions in v1? reuse the existing control-plane API (#64) or new endpoints? is this the show/demo management screen or the production admin console? <br>**Scoped + building 2026-07-01 (Eddie): node-tree console with real hardware facts (cpu/mem/gpu/storage/networking).** Arch settled in **ADR-0018** — the node container is the sole security boundary; **default = read-only facts** (via open-source ansible `setup`, **Option A**: driven *through* the container), **owner-enabled = read/write + resources** (first = LLM inference), enable rides the #65 per-device/workgroup seam. Extends `clients/console` (`bard-pro-console`), not a new frontend. Plan: `plans/PLAN_fleet_facts_console.md` (S1 ansible capture ✅; S2 projector + `NodeFacts` contract; S3 `GET /nodes`; S4 console tree; S5 live-verify + §14 screenshots). Also resolves #60's storage-discovery gap via facts, not by extending power-profile. Base-image calls (ADR-0018 amendments): facts posture = Project Hummingbird/ubi-micro; GPU/inference posture = `nvidia/cuda` Ubuntu (scoped §6 exception). | 2026-06-28 | In Progress — 2026-07-01: S1–S4 done+committed (2daf4e4 backend 663 tests 100%; 05d3455 console 27 e2e). Remaining: S5 live-wire + §14 screenshots. |
